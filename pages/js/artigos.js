...
function loadArticle() {
    ...
    db.collection('artigos').doc(idartigo).get()
        .then(function(dataarticle) {
            ...
            var dt = art.data.split(' '); // ObtÃ©m data [0] e hora [1]
            var ds = dt[0].split('-'); // ObtÃ©m ano [0], mÃªs [1] e dia [2]
            var data = `${ds[2]}/${ds[1]}/${ds[0]} Ã s ${dt[1]}`;
        ...
        .catch(function() {
            $('#artConteudo').html('Artigo nÃ£o encontrado.');
        });

}

db.collection("artigos").where('status', '==', 'ativo').orderBy('data', 'desc').get() // LÃª os dados da coleÃ§Ã£o
    .then(function(querySnapshot) { // Se conseguiu:

        querySnapshot.forEach((doc) => { // Loop para obter cada artigo

            // ObtÃ©m os dados do artigo
            var art = doc.data();
...
<p class="center">VocÃª precisa se logar para ver os artigos.</p>
<p class="center">
...
Somente leitura dos artigos para usuÃ¡rios logados:
...
Acesso total e pÃºblico aos artigos:
